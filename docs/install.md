...
1. Download a Darwin (macOS) Release: https://github.com/observIQ/bindplane-op/releases/download/v0.5.0/bindplane-v0.5.0-darwin-arm64.zip
2. Extract the zip file
3. Run `bindplanectl` from your terminal.
4. Optionally copy `bindplanectl` to your path: `sudo cp bindplanectl /usr/local/bin`
...
After running the collector install command, the collector will appear in the web interface under the "agents" tag.
